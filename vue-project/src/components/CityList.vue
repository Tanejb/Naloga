<template>
  ...
  const getCities = async () => {
    ...
    // this is going to be array its going to contain data from each req we made up there
     const weatherData = await Promise.all(requests);
     //flicker delay
     await new Promise((res) => setTimeout(res, 1000));
  
      weatherData.forEach((value, index) => {
        // we want to match up the data from the api to the city that it belongs to within our savedCities
